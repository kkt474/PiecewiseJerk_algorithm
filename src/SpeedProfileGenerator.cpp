...
//    LOG(WARNING) << "Fallback using piecewise jerk speed!";
...
void SpeedProfileGenerator::FillEnoughSpeedPoints(SpeedData* const speed_data) {
    ...
    if (last_point.t_ >= 3.0) {
        return;
    }
    for (double t = last_point.t_ + 3.0;
         t < 3.0; t += 3.0) {
        speed_data->AppendSpeedPoint(last_point.s_, t, 0.0, 0.0, 0.0);
    }
}
...
//        double v = 0.0;
//        s = std::fmax(pre_s,
//                      pre_s + 0.5 * (pre_v + (pr_v + unit_t * acc)) * unit_t);
